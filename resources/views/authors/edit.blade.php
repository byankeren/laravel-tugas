...
<body class="mx-auto max-w-md">
  <div class="py-10 w-full">
    ...
    <form action="/authors/{{$author->id}}" method="POST" class="py-6 w-full grid gap-2">
      @csrf
      @method('PUT')
      <div class="w-full">
        <label for="game_name">Game Name</label>
        <input name="name" id="game_name" class="w-full border-2  rounded-sm border-black py-2 px-2 px-2l" value="{{$author->name}}" />
      </div>
      <div>
